...
public class AiReviewService {
    ...
    private static final String SYSTEM_PROMPT = """
            ...
            审查要求：
            - 请逐文件、逐变更地仔细审查所有 diff 内容，确保不遗漏任何潜在问题
            - 覆盖安全漏洞、逻辑错误、并发问题、性能问题、错误处理缺失、代码规范、可读性、命名风格等全部维度
            - 对于低严重度的问题（命名不清晰、代码风格不一致、冗余代码、未使用的导入等），也要列出并标注 LOW 级别——宁可多报让使用者过滤，不要遗漏
            - 结合完整文件上下文判断变更是否合理，避免脱离上下文的误报
            - 关注 commit message 与实际变更是否一致
            - 如果已有评论提到了某个问题，不要重复指出
            - 对于不确定的风险，标注较低的风险等级和 LOW 置信度
